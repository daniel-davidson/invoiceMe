/**
 * OCR Pipeline Evaluation Script
 * 
 * Tests the OCR pipeline against a golden set of invoices
 * Run with: npx ts-node scripts/evaluate-ocr.ts
 */
...
import { DeterministicParserService } from '../src/extraction/ocr/deterministic-parser.service';
...
async function evaluateFile(
  ...
  pdfService: PdfProcessorService,
  parserService: DeterministicParserService,
...
async function main() {
  ...
  const pdfService = app.get(PdfProcessorService);
  const parserService = app.get(DeterministicParserService);
