...
async function main() {
  ...
  const createdVendors: Array<{ id: string; name: string }> = [];
  ...
  // Create invoices with various dates and currencies
  const invoices = [
    // AWS invoices
    {
    ...
    // Office Supplies invoices (with EUR currency)
    {
    ...
    },
    {
      ...
      needsReview: true, // Flagged for review due to unusual amount
    },
    // Coffee Shop invoices (ILS currency)
    {
    ...
    // Duplicate invoice for testing anomaly detection
    {
      vendorId: createdVendors[2].id,
      ...
      needsReview: true, // Flagged as potential duplicate
    },
  ];
